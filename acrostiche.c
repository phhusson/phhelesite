...
int main(int argc, char **argv, char **envp) {
	...
	char *cmd;
	asprintf(&cmd, "curl http://acrostiche.org/telesite/animaux 2>/dev/null");
	FILE *fd=popen(cmd, "r");
	...
	while(!feof(fd)) {
		...
		*(buffer+strlen(buffer)-1)=0;
		...
		char *tmp;
		while((tmp=index(buffer2, '\xe8'))) tmp[0]='e';
		while((tmp=index(buffer2, '\xe9'))) tmp[0]='e';
		printf("<a href='acrostiche2.cgi?%s&%d'>%s</a>&nbsp;<br/>&nbsp;\n", buffer2, version, buffer);
		bzero(buffer, 2048);
	}
	printf("\t\t</tr></td></table>\n");
	if(version==0) {
		printf("\t\t<table width=600 border=0 cellpadding=8 cellspacing=0 align=center>\n");
		printf("\t\t<tr><td><FONT family=\"Symbol\" color=\"#FF000020\"size=5>\x29</FONT>&nbsp; Accueil&nbsp;</td>");
		printf("\t\t</table>\n");
	}
	printf("\t</body></html>\n");
	pclose(fd);
